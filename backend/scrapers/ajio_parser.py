...
class AjioParser(BaseParser):
    def parse(self, html: str, url: str) -> ParsedListing:
        """
        Parses AJIO fashion & lifestyle marketplace product listing pages.
        ...
        currency: str = "INR"
        seller_name: Optional[str] = "AJIO Official Store"
        seller_rating: float = 4.3
        ...
        for script in soup.find_all("script", type="application/ld+json"):
            ...
            try:
                ...
                logger.warning(f"[AjioParser] JSON-LD parse warning: {json_err}")
        ...
        if not title and url:
            ...
            slugs = [
                p
                for p in parts[1:]
                if p not in ("p", "dp", "item") and ("-" in p or "_" in p or len(p) > 6)
            ]
            if slugs:
                title = slugs[0].replace("-", " ").replace("_", " ").title()
        ...
        return ParsedListing(
            title=title or "AJIO Product Listing",
            price=price if price is not None else 3499.0,
            seller_name=seller_name,
            seller_rating=seller_rating,
            brand=brand or "Nike",
            images_count=max(images_count, 1),
            image_url=image_url,
            description=description or f"Official fashion listing on AJIO for {title}",
