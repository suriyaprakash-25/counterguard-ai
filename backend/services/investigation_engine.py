...
class InvestigationEngine:
    ...
    def run(self, listing_id: str, listing_data: JSONDict = None) -> InvestigationState:
        ...
        try:
            ...
            logger.info(
                f"Investigation finished for listing_id: {listing_id} in {execution_time:.4f} seconds"
            )
            return final_state
            ...
            execution_time = time.time() - start_time
            logger.error(
                f"Investigation failed for listing_id: {listing_id} after {execution_time:.4f} seconds. Error: {str(e)}"
            )
