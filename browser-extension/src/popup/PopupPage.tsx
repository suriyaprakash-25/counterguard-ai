...
export function PopupPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setHistoryList((prev) => [newHistoryItem, ...prev.filter((h) => h.investigationId !== newHistoryItem.investigationId)]);
    } catch (err) {
  ...
  const getFilteredHistory = (): InvestigationHistoryItem[] => {
  ...
  return (
    <div className="w-[420px] bg-slate-950 text-white min-h-[640px] flex flex-col font-sans border border-slate-800 shadow-2xl">
      ...
      {/* ── Tab Switcher Header (Threat Inspection vs Investigation History) ──── */}
      <div className="flex border-b border-slate-800 bg-slate-950 text-[10px] font-mono font-bold">
        <button
        ...
        >
          <Search className="h-3 w-3" /> Threat Inspection
        </button>
        <button
        ...
        >
          <HistoryIcon className="h-3 w-3" /> History ({historyList.length})
        </button>
      </div>
      ...
      {activeTabNav === "INSPECT" ? (
        ...
        /* ── INVESTIGATION HISTORY TAB VIEW ───────────────────────────────── */
        <main className="flex-1 p-3.5 space-y-3 overflow-y-auto max-h-[500px] animate-fadeIn">
          {/* Search & Filter Toolbar */}
          <div className="space-y-2 bg-slate-900 p-2.5 rounded-xl border border-slate-800 font-mono text-[10px]">
            <div className="flex items-center gap-2 bg-slate-950 px-2.5 py-1.5 rounded-lg border border-slate-800">
              <Search className="h-3.5 w-3.5 text-slate-400 shrink-0" />
              <input
                type="text"
                placeholder="Search history by title, seller, marketplace..."
                value={historySearch}
                onChange={(e) => setHistorySearch(e.target.value)}
                className="bg-transparent text-white w-full focus:outline-none placeholder-slate-500 text-[10px]"
              />
              {historySearch && (
                <button onClick={() => setHistorySearch("")} className="text-slate-500 hover:text-white font-bold">×</button>
              )}
            </div>
            ...
            <div className="flex items-center justify-between gap-1">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  className="p-1 rounded bg-slate-800 hover:bg-slate-700 text-blue-400 disabled:opacity-50 transition-colors"
                ...
                <button
                  ...
                  disabled={historyList.length === 0}
                  className="p-1 rounded bg-slate-800 hover:bg-red-900 text-red-400 disabled:opacity-50 transition-colors"
          ...
          {filteredHistory.length === 0 ? (
            <div className="p-8 text-center bg-slate-900/50 rounded-xl border border-slate-800 space-y-2">
              <HistoryIcon className="h-8 w-8 text-slate-600 mx-auto" />
              <p className="text-xs text-slate-400 font-mono">No stored investigation records found.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {filteredHistory.map((item) => (
                <div
                  key={item.id}
                  className="p-3 rounded-xl bg-slate-900 border border-slate-800 space-y-2 hover:border-slate-700 transition-colors"
                >
                  <div className="flex items-center justify-between">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-white truncate max-w-[340px]">
                      {item.productTitle}
                    </h3>
                  ...
                  <div className="flex items-center justify-between pt-1 border-t border-slate-800 text-[9px] font-mono">
                    <span className="text-slate-500 truncate max-w-[170px]">INV: {item.investigationId}</span>

                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => handleOpenDashboardReport(item.investigationId)}
                      ...
                      >
                        <RotateCcw className="h-2.5 w-2.5" /> Reopen in Dashboard
                      </button>
                      <button
