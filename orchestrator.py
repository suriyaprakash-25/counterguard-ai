...
def log_event(
    ...
    state["confidence_score"] = max(
        0.0, min(100.0, state["confidence_score"] + confidence_delta)
    )
...
# ---------------------------------------------------------------------------
def cross_query(
    state: InvestigationState, asking_agent: str, target_agent: str, question: str
) -> str:
...
def text_consistency_agent(state: InvestigationState) -> InvestigationState:
    ...
    log_event(
        ...
        "text",
        "spec_check",
        "Title/spec consistent with catalog",
        confidence_delta=0,
    )
    return state
...
def seller_network_agent(state: InvestigationState) -> InvestigationState:
    ...
    answer = cross_query(
        ...
        "graph",
        "visual",
        "These three sellers appear related. Can you compare logos across all three?",
    )
    if "92%" in answer:
        log_event(
            state,
...
def build_graph():
    ...
    g.add_conditional_edges(
        "fusion", route_after_fusion, {"legal": "legal", "end": END}
    )
...
if __name__ == "__main__":
    ...
    for e in final_state["evidence_timeline"]:
        print(
            f"{e['timestamp']}  [{e['agent']:>15}]  {e['action']:<20}  {e['detail']}"
